...
This represents a departure from prior work, where distributed control enabled simple tasks to be addressed through emergent cluster behaviors.  For example, in \cite{o2010self}, Swarm-Bots self-assemble into random structures that allow them to ascend a slope too steep for a single robot.  In contrast, our system's centralized architecture facilitates the use of more complex morphologies with specialized functionalities, and sophisticated decision-making that links high-level tasks to configurations and behaviors that can complete them.
