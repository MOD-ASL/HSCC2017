...
Note that each library entry consists of a configuration with its available behaviors subject to restrictions in the environment. Thus, environment conditions and the robot's perception of the environment play important roles in the design and use of the library. The library must be designed to provide configurations that can perform desired tasks in all environment conditions the robot expects to encounter. During performance of a task, the robot must use perception to characterize its environment in terms of each configuration's capabilities. these characterizations can then be used to choose the appropriate configuration for each portion of the task. This environment characterization method is described in the Section \ref{sec:environment-characterization}.

In this work, we create five library entries for two different configurations,
listed in Table~\ref{table:1}. This library was created to accomplish the task described in Section~\ref{sec:experiments}, but the library can be expanded to include more entries for other environments and tasks.
