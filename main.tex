...
Our system is built around the SMORES-EP modular robot, described in detail in Section \ref{sec:hardware}. Because SMORES-EP is capable of changing its physical morphology through self-reconfiguration, we must provide appropriate abstractions of the robot that allows our high-level planner to address tasks.  These abstractions are described in Section \ref{sec:configuration-specifics}.

The modularity of our robot makes sensing challenging.  We have extended the existing SMORES-EP hardware system by developing a sensor module which can be carried as part of a cluster of SMORES-EP modules to provide sensing and localization information, described in detail in Section \ref{sec:sensor_module}.

Our system is intended to address tasks where the robot must explore an \emph{a priori} unknown environment and react to what it encounters.  To that end, we provide a suite of software tools for sensing and perception. Our perception tools allow a SMORES-EP robot to perform SLAM, navigation, and obstacle avoidance using the sensor module.  This software has been specifically optimized to work with data from lightweight onboard depth camera, and to run efficiently using the limited computation power of the onboard computer.  Additionally, a novel next-best-view algorithm automatically selects waypoints in the environment to allow the robot to maximize information gain as it explores.  These tools are discussed in detail in Section \ref{sec:exploration}.
...
Finally, we provide software and hardware tools enabling rapid, robust, autonomous self-reconfiguration. Our system allows SMORES-EP to self-reconfigure without external localization, using overhead camera carried by the robot to track AprilTag fiducials mounted to the modules.
%Our reconfiguration system also relies on elements of our perception algorithms, which provide goal pose information for docking.
Our reconfiguration tools are described in Section \ref{sec:reconfiguration}.

After presenting each component, we demonstrate how they can operate as an integrated system capable of autonomously solving high-level tasks in unknown environments.  In Section~\ref{sec:experiments}, we demonstrate completion of a realistic object retrieval task in an unknown environment with full autonomy.  Finally, in Section~\ref{sec:discussion}, we evaluate the success of our system in achieving its goals, and conclude.
%
...
%
\subsection{Characterization of Environment Properties}
