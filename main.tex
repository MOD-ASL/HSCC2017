...
%
In most MSRR systems, individual modules have very limited sensing capability, so autonomous reconfiguration is accomplished by including a sensor or camera module as a component of each cluster of modules in the system.  In our system, the sensor module (Figure~\ref{fig:sensor-module}) is capable not only of determining the position of individual modules relative to the cluster, but also includes an Xtion-Pro RGB-D camera that facilitates SLAM in an unknown environment.
Onboard computation is provided by an UP-board \TODO{cite and get specs}, which communicates with the ROS core over 802.11 wifi.  The sensor module is self-powered, with a battery life of about 1.5 hours.

% Sensor Module Figure
\begin{figure}
\begin{center}
\includegraphics[height=1.5in]{images/sensor_module.jpg}
