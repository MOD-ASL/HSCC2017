...
%                        /____/        /_/
{\color{red} 
\textbf{Library of Configurations and Behaviors}
...
Users can create more entries to the library with a physics-based simulator program VSPARC (Verification, Simulation, Programming And Robot Construction \footnote{\url{www.vsparc.org}}).
Created behaviors can then be utilized to control the SMORES-EP robot system.
In addition, each library entry is labeled with attributes that describe the behavior's capabilities and environment conditions. 
...
In our mail delivery experiment, the task specifications require the robot to reach to a mailbox and the environment characterization algorithm reports that the mailbox is in a ``stairs''-type environment.
Then the high-level planner search the design library for a configuration and behavior that is capable to climb stairs.
