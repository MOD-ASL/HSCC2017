...
The system is reactive: it decides what action to perform based on the sensed environment, and may choose different actions to complete the same task in different environments.
...
The hardware consists of a set of \textbf{Robot Modules} (which can move, and connect to one another) and a single \textbf{Sensor Module} (which includes environment sensors and a computer).
...
The \textbf{Perception and Exploration} component builds a map of the environment, and directs the robot to move and explore. 
As it explores, the \textbf{Environment Characterization} component identifies features of the environment relevant to the high-level task, transforming raw sensor data into discrete attributes.
Based on the task requirements and sensed environment, the high-level planner will select an appropriate behavior for the robot from the \textbf{Library}.
...
I2C & 3 & 12.5\% \\ 
\hline
Bad calibration & 4 & 16.7\% \\ 
\hline
Broken Hardware & 3 & 12.5\% \\ 
\hline
Navigation stack is stupid & 3 & 12.5\% \\ 
\hline
didn't see pink & 2 & 8.3\% \\ 
\hline
docking gives bad pose & 4 & 16.7\% \\ 
\hline
ROS drop messages & 1 & 4.2\% \\ 
\hline
Human Error & 4 & 16.7\% \\ 
\hline
\end{tabular}
