...
For example, consider the following mail delivery task.  The robot is asked to explore its environment, locate a mailbox, and drop an object in the mailbox.  After exploring and locating the mailbox, it recognizes that in order to reach the mailbox, it will need to ascend a flight of stairs. In response, it chooses to reconfigure into a stair-climbing configuration, which it then uses to climb the stairs and deliver the object.  Afterwards, it descends and reconfigures back into a driving configuration, to return to home base.
...
Based on the environment characterization and target location, the algorithm will also return a waypoint for the robot to position itself to perform its task (or to reconfigure, if necessary).  In our mail delivery example, the environment characterization algorithm would direct the robot to drive to a waypoint at the base of the stairs, which is the best place for it to reconfigure and begin climbing the stairs.
...
In three hardware experiments, we demonstrate how our system allows the robot to autonomously explore, decide how and when to reconfigure, and manipulate objects to complete tasks in unknown environments. Table \ref{table:task-compare} illustrates the environments and tasks in each experiment.
...
\hline
\raisebox{-.5\height}{\includegraphics[width=0.25\textwidth]{images/overhead_starting.jpg}} & Explore environment to find all pink or green objects and blue dropoff zone. Deliver all objects to dropoff zone.\\ 
\hline
\raisebox{-.5\height}{\includegraphics[width=0.25\textwidth]{images/stairs_explore_overhead.jpg}} & Explore environment to find mailbox, then deliver a circuit to the box. \\ 
\hline
\raisebox{-.5\height}{\includegraphics[width=0.25\textwidth]{images/stamp_explore_overhead.jpg}} & Explore environment to find package, then place a stamp on the package. \\ 
\hline
\end{tabular}
\caption{Tasks Comparison}
...
Figure \ref{fig:exps}a\TODO{Change the reference here and maybe talk about reconf?} shows the environment setup for Experiment II, which includes an obstacle (cardboard box), a staircase, and the mailbox at the top of the staircase.  The mailbox is marked with a pink rectangle.  The astute reader will recognize that this scenario is identical to the mail delivery example referred to throughout the paper.  
