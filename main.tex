...
We present the first modular robot system capable of autonomously completing high-level tasks by reactively reconfiguring to meet the needs of a perceived, \textit{a priori} unknown environment.  The system integrates perception, high-level planning, and modular hardware tools. Based on a high-level task specification, the modular robot autonomously explores an unknown environment using a depth camera to search for objects or regions of interest. When such objects are discovered, perception tools characterize the environment to inform the high-level planner when and how to reconfigure the robot morphology and to manipulate objects to complete its task. This work represents an extension of the current state of the art in autonomous modular robots to enable intelligent reconfiguration in response to perception of unknown environments.
%
\end{sciabstract}
...
Continued research in  MSRR has resulted in substantial advancement.  Existing research has demonstrated MSRR self-reconfiguring, assuming interesting morphologies, and exhibiting various forms of locomotion, as well as methods for programming, controlling, and simulating modular robots \cite{Yim2007, Rubenstein2004,Murata2006,Paulos2015,Yim1994,Jing2016, fukuda1990cellular, murata1994self, chirikjian1994kinematics, dutta2018distributed,ryland2010design, wolfe2012m, romanishin20153d, mantzouratos2015embeddability}.
...
We validate our system in three hardware demonstrations, showing that, based on a high-level task specification, the modular robot autonomously explores an unknown environment, decides if, when, and how to reconfigure, and manipulates objects to complete its task.
...
We selected the three scenarios in our demonstrations to showcase a range of different ways SMORES-EP can interact with environments and objects: movement over flat ground, fitting into tight spaces, reaching up high, climbing over rough terrain, and manipulating objects.  This broad range of functionality is only accessible to SMORES-EP by reconfiguring between different morphologies.
...
\TODO{Add a paragraph/sentence on limitations of perception}
...
To aid users in designing configurations and behaviors, we created a open-source computer design tool called Verification, Simulation, Programming And Robot Construction (VSPARC\footnote{\url{www.vsparc.org}}) and made it available online.
...
\TODO{Reviewer 2 points out that we don't actually say what the high level planner \emph{is} in the section. Be sure to cite the autonomous robots paper and provide a one-sentence summary of what the planner is.}
...
The configurations and behaviors used to complete this task are not directly specified by the user, but instead determined by the high-level planner as it continually reacts to the sensed environment.
...
\caption{Specification for dropping an object in the mailbox. \TODO{Jim, can you change top caption to "Drop an object in the mailbox"?}}
\label{fig:spec}
\end{subfigure}
