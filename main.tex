...
In MSRR systems, sensing and processing capabilities of individual modules are limited by the size and weight constraints of the module form factor. Therefore, the presented system uses a special sensor module dedicated to sensing and processing equipment, shown in Figure \ref{fig:sensor-module}. The module has no actuation capabilities and is larger than SMORES-EP modules. It is equipped with a front-facing Xtion Pro Live RGB-D sensor which enables the robot to explore and map the environment and recognize objects of interest. A high, downward-facing HD webcam is included to provide a view of the robot itself, which is used for self-reconfiguration. Finally, an UP computing board provides high performance I/O and processing capability in a small form factor. The UP Board used in the presented system has an Intel Atom 1.92 GHz processor, 4 GB memory, and a 64 GB hard drive. It is network connected via 802.11 wifi. A battery provides power to the Up Board with a lifetime of about 1.5 hours.
...
up and dropping items in ``free'' environment while driving on a flat plain as a
...
\section{Characterization of Environment Features}
...
To bring together the actuation, sensing, and reconfiguration capabilities of  the  robot as a complete system for accomplishing  tasks, we employ an existing framework called LTLMoP to automatically generate robot controllers from user-specified high-level instructions using formal methods%\TODO{cite}
.
...
For example, the system proposition ``pickUp'' is \lt if the robot is picking up an item (and \lf otherwise), and
the environment proposition ``pinkObject'' is \lt if the robot is currently sensing a pink object (and \lf otherwise).
...
For example, Spec.~\ref{spec:simple} is a Structured English task specification that directs the robot to explore a room in search of a pink object, and pick it up once it is located. In addition to the ``pickUp'' and ``pinkObject'' propositions, this spec uses the system propositions ``explore'' and ``driveToObject,'' each of which represents a different robot action. ``arrived'' is an environment proposition that is \lt if the robot arrives at the pink object.
...
Given a spec,  LTLMoP will synthesize a robot controller that satisfies the given robot task (if one exists). The controller synthesized from Spec.~\ref{spec:simple} is shown in Fig.~\ref{fig:automaton} in the form of a finite state automaton. Each state is labeled with the values of all system propositions, with the ``!'' prefix denoting propositions that are $\lf$. Each transition between two states is labeled with the value of {\tt some} environment propositions. Some states and transitions are omitted for clear presentation. %\TODO{{\tt some}?}
...
For example, to implement the ``pickUp'' proposition in Spec.~\ref{spec:simple}, the system can execute any behavior that has the $pickUp$  behavior property, and which also satisfies the current environment properties.  If the current robot configuration cannot execute an appropriate behavior, the system will reconfigure to a different configuration that can.   
...
The results of the experiment validated our system's ability to adapt to unknown environments to complete high-level tasks. They also revealed several insights into challenges faced by autonomous modular systems, and ways to improve such systems in the future.

\subsection{Challenges}
Some features specific to modular systems create challenges for performing high-level tasks using perception-informed autonomy. The processing power required for perception and environment characterization presented issues with size and computation speed. As can be seen from Figure~\ref{fig:sensor-module}, the sensor module is significantly larger than a regular module in order to hold the required sensors and computer board. This imposes significant limitations on possible configurations and their locomotion abilities, due to the low connection strength and power of modules. At the same time, the small computer board used in the sensor module results in low computation power, which causes the sensor processing and perception algorithms to run slowly and have less accuracy. Simple configuration designs and efficient algorithms were used to address these issues. Future autonomous modular systems would benefit significantly from powerful but highly compact, lightweight sensing and processing components that can be carried onboard the robot while imposing minimal size and weight constraints.

In our experiment, robot speed was very low due to power limitations of individual modules. In addition, having a robotic system composed of 6 individual robotic components (5 SMORES-EP modules and 1 sensor module) multiplied the chance of an individual component having a hardware failure. The long runtime combined with multiplied failure points created a high risk of an error occurring during the experiment. To make the system more robust to these errors, repetitive feedback controllers and checks were implemented during task performance. When the robot navigates toward and retrieves objects, it constantly updates its estimate of the object location as new measurements of the object are received. During reconfiguration, docking modules are checked and re-aligned several times during the docking process to ensure they are lined up precisely before connecting to each other. This helps the robot recover from controller lag or a module not responding to velocity commands.
%
