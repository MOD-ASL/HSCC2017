...
For the final experiment, the robot is given a postage stamp to place on the package so that it can be shipped to Cornell. The high-level task has the same specification as in Experiment II: explore the unknown environment, find the target location indicated by a pink area, and place the object at the target.
