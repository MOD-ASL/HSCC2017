...
\author{Jonathan~Daudelin,~\IEEEmembership{Member,~IEEE,}
        ...
        Tarik~Tuson,~\IEEEmembership{Member,~IEEE,}
...
\thanks{T. Tuson and M. Yim are with the Mechanical Engineering and Applied Mechanics Department, University of Pennsylvania, Philadelphia, PA, 19104.}% <-this % stops a space
...
To identify objects of interest in the task (such as the dropoff zone), color detection and tracking is used.  Colored objects are recognized using CMVision\footnote{CMVision: http://www.cs.cmu.edu/$\sim$jbruce/cmvision/}, and tracked in 3D using depth information from the onboard 3D sensor\footnote{Lucas Coelho Figueiredo: https://github.com/lucascoelho91/ballFollower}. For the example task, the dropoff zone is assumed to have a pink sign on it to enable the robot to recognize it in the environment. Although we implement object recognition by color, more sophisticated methods could be included in the same system architecture.
